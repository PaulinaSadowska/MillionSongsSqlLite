package oldStructure.TableManager;

import dataObjects.oldScheme.UniqueTrack;
import oldStructure.DatabaseManager;
...
public class UniqueTrackTableManager implements ITableManager<UniqueTrack>
{
    ...
    public boolean createTable(Statement statement)
    {
        final String createListenRecordQuery =
                "CREATE TABLE IF NOT EXISTS " + DatabaseManager.UNIQUE_TRACKS_TABLE + " (performanceId varchar(18), songId varchar(18), " +
    ...
    public List<UniqueTrack> selectAll(Statement statement)
    {
        ...
        try {
            ResultSet result = statement.executeQuery("SELECT * FROM " + DatabaseManager.UNIQUE_TRACKS_TABLE);
            while(result.next()) {
    ...
    private boolean insertRecord(UniqueTrack uniqueTrack, Connection connection)
    {
        try {
            PreparedStatement prepStmt = connection.prepareStatement(
                    "insert into " + DatabaseManager.UNIQUE_TRACKS_TABLE + " values (?, ?, ?, ?);");
