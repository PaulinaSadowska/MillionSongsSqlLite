...
public class ListenTableManager implements ITableManager<ListenRecord>
{
    ...
    public boolean createTable(Statement statement)
    {
        final String createListenRecordQuery =
                "CREATE TABLE IF NOT EXISTS " + DatabaseManager.LISTEN_RECORD_TABLE + " (_id INTEGER PRIMARY KEY AUTOINCREMENT, songId varchar(18), userId varchar(18), timestamp INTEGER);";
        try {
    ...
    public List<ListenRecord> selectAll(Statement statement)
    {
        ...
        try {
            ResultSet result = statement.executeQuery("SELECT * FROM " + DatabaseManager.LISTEN_RECORD_TABLE);
            while(result.next()) {
    ...
    private boolean insertRecord(ListenRecord listenRecord, Connection connection) {
        try {
